...
| [动手：GRPO 训练与核心机制](./grpo-practice-and-mechanism)                            | GRPO 怎么用组内归一化替代 Critic？省掉 Critic 后显存能省多少？          | 能跑通 GRPO 训练，理解它和 PPO 的关键差异     |
| [DeepSeek、DAPO 与 RLVR](./deepseek-dapo-rlvr)                                        | 纯 RL 训练不用 SFT 行不行？可验证奖励能取代 RM 吗？                     | 理解 RLVR 的训练范式和 DeepSeek-R1 的核心实验 |
| [RL Scaling 与前沿展望](./rl-scaling-outlook)                                         | Online vs Offline 怎么选？RL Scaling 的天花板在哪？                     | 建立对 RL 训练范式的全局判断力                |
| [知识蒸馏——从大模型到小模型的知识迁移](../chapter08_grpo_rlvr/on-policy-distillation) | 蒸馏为什么对小模型比 RL 更有效？Teacher 的 log-prob 怎么当 reward？     | 理解蒸馏的 RL 本质和工业界实践                |
