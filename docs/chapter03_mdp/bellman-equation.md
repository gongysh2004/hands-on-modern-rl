...
上面的公式假设策略只选一个固定的动作。但实际上，策略 $\pi$ 可能以不同的概率选择不同的动作——比如"70% 拉 A，30% 拉 B"。把所有动作都考虑进来，完整的贝尔曼期望方程是：
...
因为策略永远选"拉 A"，外层的 $\sum_a \pi(a|s)$ 只有一项。而且老虎机是单状态的——不管拉多少轮，每一轮面对的情况完全一样，所以 $V$ 在每一轮都相同。
...
下一节，让我们看看 RL 发展历程中的三代经典方法——它们是怎么把贝尔曼方程和 TD Error 变成能跑的算法的，以及当状态太多时怎么办。[经典方法与路线图](./classic-methods)。
