# 第 10 章：Agentic RL——工具调用、多轮交互与智能体训练
...
至于"怎么给这个 Agent 加上 RL 训练"——我们需要解决奖励怎么算（ORM vs PRM）、多步交互中奖励怎么分配（信用分配）、训练数据怎么管理等问题。这些正是后续章节要展开的，我们在 [多轮交互与信用分配](./multi-turn-rl) 的动手实验中会把这个简单循环扩展成一个可训练的 RL 系统。
...
这些框架的核心差异可以归结为一个取舍：**同步 vs 异步**。同步训练是"一桌吃完再上下一桌"——简单、可控、容易调试，但 GPU 利用率低。异步训练是"所有桌子同时开"——吞吐翻倍，但训练数据可能基于旧权重生成，需要额外的算法补偿。AReaL 的研究表明，异步训练可以在不损失效果的前提下将速度提升近 3 倍——但前提是你的训练已经调通了。另一个极端是 Seer：它坚持同步框架，不改变 GRPO 算法，而是通过在线上下文学习（divided rollout、context-aware scheduling、adaptive grouped speculative decoding）消除 rollout 长尾延迟，在保持 on-policy 保证的前提下将吞吐提升 74–97%（[arXiv:2511.14617](https://arxiv.org/abs/2511.14617)）。
...
框架的选型取决于你的具体场景。如果刚入门想快速跑通 demo，OpenRLHF 代码最简洁、文档最完善。如果是企业级大规模训练（70B+），verl 的吞吐和生态优势明显。如果模型是 MoE 架构（如 GLM-4.5、Qwen3-30B-A3B、DeepSeek-R1），slime 的 Megatron + SGLang 原生架构对 MoE 的 fp8 rollout、DeepEP 通信等做了专门优化，效率最高。如果要极致吞吐，AReaL 的全异步模式能做到近 3 倍加速。更多工程细节——沙箱管理、环境构建、分布式部署——我们将在 [工具调用与 Agentic 工程](./tool-use-and-trajectory) 中展开。
...
- [GRPO 与 RLVR](../chapter09_grpo_rlvr/rlvr)——"可验证奖励"是 Agentic RL 的天然选择
...
| [项目二：Deep Research Agent](./deep-research-agent)           | 长程搜索、引用验证、报告生成和 Deep Research RL 方案                          |
