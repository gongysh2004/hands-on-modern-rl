# 4.1 从 Q-Learning 到 DQN

在第 3 章的末尾，我们在 4×4 的 GridWorld 上手算了 Q-Learning。16 个格子，4 个方向，一共 64 个 Q 值。初始全是 0，每走一步更新一次，跑了 1000 个 episode 后 Q 值收敛，智能体学会了从起点到终点的最短路径。整个过程中，我们维护了一张表格——每个状态-动作对占一行，逐行更新。表格虽小，却精确地编码了"每个局面下应该怎么走"的知识。

这张表格是 Q-Learning 的全部记忆，也是它最大的软肋。
...
这就是 DQN——Deep Q-Network。名字里的"Deep"指的是深度神经网络。

具体来说，DQN 的神经网络接收一个状态 $s$ 作为输入，同时输出所有可能动作的 Q 值。比如在 CartPole 中，网络接收 4 维状态向量，输出 2 个 Q 值（对应"左推"和"右推"）。在 Atari 中，网络接收 84×84×4 的像素帧，输出 4 到 18 个 Q 值（取决于游戏有多少个可用动作）。
...
这两个问题让"直接把神经网络套在 Q-Learning 上"这个看似简单的方法在实践中完全无法工作。DeepMind 的贡献不在于"用神经网络代替表格"这个想法本身——这个想法早在 1990 年代就有人提过了。他们的贡献在于提出了两个精巧的工程技巧来解决上述两个问题：**经验回放**（Experience Replay）打破样本相关性，**目标网络**（Target Network）稳定训练目标。这两个技巧让 DQN 真正跑了起来，也让 DeepMind 的论文在 2015 年登上了 Nature 的封面 [^1]。
...
DeepMind 的 DQN 在 2013 年首次公布（arXiv 论文），2015 年正式发表在 Nature 上。这两篇论文标志着深度强化学习时代的开端。

</details>

接下来，让我们深入拆解 DQN 的三个核心组件——[Q-Network、经验回放和目标网络](./dqn-components)。
