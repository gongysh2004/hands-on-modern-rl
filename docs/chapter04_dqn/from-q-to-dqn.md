...
LunarLander 的状态是 8 维连续向量，包括位置、速度、角度、角速度和两条支架是否接触地面。前 6 个维度都是实数，理论上存在无穷多个不同状态。即便我们只把其中 6 个连续维度各离散化成 50 个格子，状态总数也有 $50^6 \approx 1.56 \times 10^{10}$，再乘上 4 个动作，就是数百亿个状态-动作格子。这还只是一个低维控制任务。
...
出路是**函数逼近**（function approximation）。核心思想来自一个朴素的观察：在现实世界中，相似的状态通常有相似的价值。LunarLander 中，飞船角度差 0.01、垂直速度差一点点的两个局面，哪个动作更好，答案大概率是相近的。表格方法把每个状态当作独立的实体，完全放弃了这种相似性；函数逼近则反过来利用它。
...
DQN 的架构很直接：网络接收状态 $s$ 作为输入，同时输出所有可能动作的 Q 值——一个输入，多个输出。在 LunarLander 中，输入是 8 维状态向量，输出是 4 个 Q 值（不喷、左侧喷口、主发动机、右侧喷口）。在 Atari 中，输入是 $84 \times 84 \times 4$ 的像素帧，输出是 4 到 18 个 Q 值（取决于游戏有多少个可用动作）。

```mermaid
flowchart LR
    State[“状态 s<br/>（8维向量 / 像素帧）”] --> NN[“神经网络<br/>Q(s, ·; θ)”]
    NN --> Q1[“Q(s, 动作1)”]
    NN --> Q2[“Q(s, 动作2)”]
    NN --> Qn[“Q(s, 动作n)”]
...
经验回放（experience replay）是解决这个问题的直接方案。下图展示了它的核心机制：把交互经验存入一个固定容量的环形缓冲区，每次训练时从中随机采样一批，而不是按顺序取最近几条。
